...
var Koji = (_class = (_temp = /*#__PURE__*/function () {
  ...
  (0, _createClass2["default"])(Koji, [{
    ...
    value: function config() {
      ...
      this.resolveMetadata(kojiConfigOptions.projectId, kojiConfigOptions.metadata); // Set up and sanity check services

      this.resolveServices(develop, deploy, kojiConfigOptions.services); // Initialize customization data

      this.remix.init(remixData);
    }
  }, {
    key: "resolveMetadata",
    value: function resolveMetadata(explicitProjectId, metadata) {
      var _this$dispatch;

      var projectId = explicitProjectId || process.env.KOJI_PROJECT_ID;
      var creatorUsername = process.env.KOJI_CREATOR_USERNAME;
      var creatorProfilePicture = process.env.KOJI_CREATOR_PROFILE_PICTURE;
      ...
      this.projectId = projectId; // Set the metadata
    ...
    key: "resolveServices",
    value: function resolveServices(develop, deploy, services) {
      ...
      if (!(0, _equalsIgnoreOrder.equalsIgnoreOrder)(deployServices, developServices)) {
      ...
      Object.keys(baseServiceMap).forEach(function (serviceName) {
        if (!baseServiceMap[serviceName]) {
          throw new Error("Unable to find a value for the ".concat(serviceName, " service. If your value is not available at `process.env.KOJI_SERVICE_URL_").concat(serviceName, "`, you may need to pass it explicitly using the second, kojiConfigOptions parameter when calling Koji.config"));
        }
      }); // Handle overrides
